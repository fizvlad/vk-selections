...
using fizvlad::vk_api::utility::getCurrentLocalTime;
...
int main() {
    ...
    try {
        ...
        bot.start([&] (Message m, bool isOld) {
            ...
            if (m.text.find("!selection ") != string::npos) {
                ...
                try {
                    ...
                    cerr << getCurrentLocalTime() << " - " << "  ERROR (API):   request: " << request << endl << "what: " << e.what() << endl;
                } catch (exception e) {
                    bot.sendMessage(m.sender, "Ой! :0\n Случилась ошибка. Попробуйте ещё раз");
                    cerr << getCurrentLocalTime() << " - " << "  ERROR (OTHER): request: " << request << endl << "what: " << e.what() << endl;
                }
                return true;
            }
            ...
            if (m.text.find("!calc ") != string::npos) {
                ...
                try {
                    ...
                    bot.sendMessage(m.sender, "Ой! :0\n Случилась ошибка. Попробуйте ещё раз");
                    cerr << getCurrentLocalTime() << " - " << "ERROR: request: " << request << endl << e.what() << endl;
                }
        ...
        cerr << getCurrentLocalTime() << " - " << "ERROR: " << e.what() << endl;
    }
